...
class JSONAPIRequest extends FormRequest
{
    ...
    public function rules()
    {
        $rules = [
            ...
            'data.relationships.*.data.type' => [Rule::requiredIf($this->has('data.relationships.*.data.id')),
            ...
            'data.relationships.*.data.*.type' => [Rule::requiredIf($this->has('data.relationships.*.data.0')),
                Rule::in(array_keys(config('jsonapi.resources')))],
        ];
